...
void MemAlloc(Particle PT,Particle pt,Parameter PM) {
...
void ABParticleField::CellPUpdate(Particle PT,Parameter PM,int i_field){
 ...
 int Nx=gridNumber().x;
    ...
    updateConcentration << <Nx,Ny>> > (PT, PM,(*ptr_Pxx).f[i_field],(*ptr_Pxy).f[i_field],f[i_field], Nx, Ny, Nbx, Nby);
