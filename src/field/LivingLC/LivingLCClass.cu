...
void LivingLC::initFields () {
    ...
    setFieldProperties(&px, name+".px", priority ,"sin");
    setFieldProperties(&py, name+".py", priority, "sin");
...
//     setFieldProperties(&Pxy, name+".Pxy", -1, init_cond);
    ...
    setFieldProperties(&C, name+".C",priority, init_cond);
