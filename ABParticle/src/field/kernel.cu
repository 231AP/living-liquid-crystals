...
void Init_Coords(int flag, Particle pt, Parameter PM) {
    ...
    for(int i=0;i<N;i++){
        ...
        i,pt.x[i],pt.y[i],cellX,cellY,PM.cellSizeX,PM.cellSizeY,xBox,yBox);
        ...
        //printf("id:%d,cell:%d,%d,cellList:%d,cellOffsetsCL:%d\n",i,cellX,cellY,pt.cellList[cellX+cellY*PM.cellNumX],pt.cellOffsetsCL[cellX+cellY*PM.cellNumX]);
    }
}
...
__device__ int getNeighborListTry(real x0, real y0, real x1, real y1, Parameter PM) {
    ...
    if (dr2 < (PM.rd+PM.rOutUpdateList) * (PM.rd+PM.rOutUpdateList))return 1;
    else return 0;
}
