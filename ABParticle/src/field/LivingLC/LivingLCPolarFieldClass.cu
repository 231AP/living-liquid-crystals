...
void LivingLCPolarField::postProcessing(int i_field) {
    ...
    (*ptr_cplus).applyBounCondPeriGPU((*ptr_cplus).f[i_field]);
    (*ptr_cminus).applyBounCondPeriGPU((*ptr_cminus).f[i_field]);
     (*ptr_C).applyBounCondPeriGPU((*ptr_C).f[i_field]);
    (*ptr_Omega).applyBounCondPeriGPU((*ptr_Omega).f[i_field]);
};
