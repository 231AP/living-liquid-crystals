...
# Generate random numbers
            ...
            # Create a single figure with multiple subplots
                    fig, axes = plt.subplots(2, 2, figsize=(18, 12))  # 2 rows and 3 columns of subplots
            # Plot Velocity PDF (fluid vs. bacteria vs. velocity)
                ...
                axes[0, 1].plot(np.arange(-50, 50) / 100 * max_velocity, PDF_velocity, label=f"fluid{id_data}")
                # axes[0, 1].plot(np.arange(-50, 50) / 100 * max_velocity, PDF_vB, label=f"bacteria{id_data}")
                axes[0, 1].legend(loc=1, prop=font)
                axes[0, 1].set_yscale("log")
                axes[0, 1].set_xlabel("Velocity", fontsize=24)
                axes[0, 1].set_ylabel("PDF", fontsize=24)
                ...
                # axes[0,0].plot(np.arange(-50, 50) / 100 * max_velocity*1.5, PDF_actForce, label=f"activeForce{id_data}")
                axes[0, 1].scatter(np.arange(-50, 50) / 100 * max_velocity*2.1, PDF_actForce*sum(PDF_actForce>0)/sum(PDF_velocity>0), label=f"activeForce{id_data}")
                axes[0, 0].legend(loc=1, prop=font)
                axes[0, 0].set_yscale("log")
                # axes[1, 2].set_xlim((0, max_F/2))
...
if __name__=="__main__":
    ...
    savedir = "../photo_video/analysis/"+ "example2"+"gammaB3-alpha-20-xi0.9-convection1-Dp0-kBT0.5"
