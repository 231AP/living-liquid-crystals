...
void Evolver::run() {
    ...
    for (time_now=time_start; time_now<=time_stop+0.1*time_step; time_now+=time_step) {
        ...
        if ((int) (time_now/time_export) > (int) ((time_now-time_step)/time_export)) {
            ...
            showProgress();
            DeviceUpdataToHost(params);//下载数据到主机

            ExpoConf(str_t,params);
    ...
    for (auto f_ptr_i : (*system_ptr).field_ptrs ) {        
        if ((*f_ptr_i).priority()>0) {
            ...
            if ((*f_ptr_i).specialty=="LaplaceNFEqField") {
                ...
                (*f_ptr_temp).ParticleToField(i_field,params);
...
void Evolver::evalFieldFuncs(Field* f_ptr_i, int i_field) {
	...
	if (f_func_i.f_func_fa_ptrs.f2_ptr != NULL) {
	    ...
	    if (f_func_i.f_func_fa_ptrs.f3_ptr != NULL) {
	        f_func_i.f_func_args.field3=(*f_func_i.f_func_fa_ptrs.f3_ptr).f[i_field];
	      	if (f_func_i.f_func_fa_ptrs.f4_ptr != NULL) {
		    f_func_i.f_func_args.field4=(*f_func_i.f_func_fa_ptrs.f4_ptr).f[i_field];
		};
	    };
	};	
...
void Evolver::fieldsUpdate(int i_f_new, int i_f_old, int i_df, double time_step_t) {
    ...
    for (auto f_ptr_i : (*system_ptr).field_ptrs ) {
	    ...
	    //   cout <<"ABParticle detected2."<<endl;
               ABParticleField* f_ptr_temp = (ABParticleField*) f_ptr_i;
                (*f_ptr_temp).ParticleToField(i_f_new,params);
