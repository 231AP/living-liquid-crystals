...
// --------------------------------------------------------------
void Field::initFieldConstMid(double f_value, double f_start=0, double f_dev=0.0000000000000001) {
    int Nx=gridNumber().x;
    int Ny=gridNumber().y;
    int Nbx=gridNumberBoun().x;
    int Nby=gridNumberBoun().y;
    uniform_real_distribution<double> randUR;
    for (int j=f_start; j<Ny/2; j++) {
        for (int i=0; i<Nx; i++) {
            int idx=(j+Nby)*(Nx+2*Nbx)+i+Nbx;
            f_host[0][idx]=f_value+f_dev*(randUR(rng)-0.5);
        };
...
// --------------------------------------------------------------
void Field::initFieldRandNorm(double mean,double vars){
    int Nx = gridNumber().x;
    int Ny = gridNumber().y;
    int Nbx = gridNumberBoun().x;
    int Nby = gridNumberBoun().y;
    ...
    for (int j=0; j<Ny; j++){
        for (int i=0; i<Nx;i++) {
            int idx =(j+Nby)*(Nx+2*Nbx)+i+Nbx;
            f_host[0][idx]=mean + vars*(randUR(rng)-0.5);
        };
    };
};//actually this function has no difference with the initFieldConst
// --------------------------------------------------------------
void Field::initFieldPhaseTwo(double range1=0.5,double alpha1=4,double alpha2=8) {
    int Nx = gridNumber().x;
    int Ny = gridNumber().y;
    int Nbx = gridNumberBoun().x;
    int Nby = gridNumberBoun().y;
...
void Field::initFieldSin(double sin_amplitude=1, int sin_period=1, double sin_phase=0) {
    ...
    for (int j=0; j<Ny; j++) {
        for (int i=0; i<Nx; i++) {
            int idx=(j+Nby)*(Nx+2*Nbx)+i+Nbx;
            double r2=(i-0.5*Nx)*(i-0.5*Nx)+(j-0.5*Ny)*(j-0.5*Ny);
            f_host[0][idx]=sin_amplitude*sin(4*M_PI*sin_period*i/Nx)+0.0001*(randUR(rng)-0.5);           
        };
    };
    applyBounCondPeriCPU(f_host[0]);
};
